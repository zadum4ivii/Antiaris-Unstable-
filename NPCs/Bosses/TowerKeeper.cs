...
namespace Antiaris.NPCs.Bosses
{
    ...
    public class TowerKeeper : ModNPC
    {
        ...
        public override void ScaleExpertStats(int playerXPlayers, float bossLifeScale)
        {
            if (playerXPlayers > 1)
            {
                npc.lifeMax = (int)(12000 + (double)npc.lifeMax * 0.2 * (double)playerXPlayers);
            }
            else
            {
                npc.lifeMax = 12000;
        ...
        {
			mod.GetModWorld<AntiarisWorld>().DownedTowerKeeper = true;
            if (Main.expertMode)
            {
                npc.DropBossBags();
            }
        ...
        public override void AI()
        {
			...
			if ((double)npc.life <= npc.lifeMax * 0.135)
            {
				...
				else
				{
					...
					if (Main.rand.Next(5) == 0)
						Projectile.NewProjectile((int)((npc.position.X - 500) + Main.rand.Next(1000)), (int)((npc.position.Y - 500) + Main.rand.Next(1000)), 0.0f, 0.0f, mod.ProjectileType("TowerKeeperSheet"), npc.damage / 3, 4.5f);
