    ...
    public class ProtectiveStone2 : ModNPC
    {
        ...
        public override bool PreAI()
        {
			...
			if ((double)npc.ai[3] % 10.0 == 1.0 && Main.rand.Next(2) == 0 && Main.netMode != 1)
			{
				...
				Projectile.NewProjectile(Main.npc[boss].Center.X, Main.npc[boss].Center.Y, start.X * speed, start.Y * speed, mod.ProjectileType("Energy"), 15, 0.0f, Main.myPlayer, Main.npc[boss].Center.X, Main.npc[boss].Center.Y - 42f);
			}
			return false;
        }
