...
namespace Antiaris.NPCs.Bosses
{
    ...
    public class AntlionQueen : ModNPC
    {
        ...
        {
			mod.GetModWorld<AntiarisWorld>().DownedAntlionQueen = true;
            if (Main.expertMode)
            {
                npc.DropBossBags();
            }
        ...
        public override void AI()
        {
            ...
            else if ((double)npc.ai[0] >= 145.0 && !this.stunned && !this.sAI)
            {
				...
				if ((double)npc.ai[2] > 60.0 && this.swarmerSpawnTimer > 60)
				{
					...
					float x = npc.Center.X + (float)Main.rand.Next(-50, 50);
					float y = npc.Center.Y + (float)Main.rand.Next(-50, 50);
					if (Main.tile[(int)(x / 16.0f), (int)(y / 16.0f)].active())
					{
						x = npc.Center.X + (float)Main.rand.Next(-50, 50);
						y = npc.Center.Y + (float)Main.rand.Next(-50, 50);
					}
					else
					{
						for (int k = 0; k < 50; k++)
						...
						if (Main.rand.Next(2) == 0) NPC.NewNPC((int)x, (int)y, 508, 0, (float)npc.whoAmI, 0.0f, 0.0f, 0.0f, 255);
						float newX = x + (float)Main.rand.Next(-50, 50);
						float newY = y + (float)Main.rand.Next(-50, 50);
						for (int k = 0; k < 50; k++)
						{
						...
						if (Main.rand.Next(2) == 0) NPC.NewNPC((int)newX, (int)newY, 509, 0, (float)npc.whoAmI, 0.0f, 0.0f, 0.0f, 255);
					}
				...
				if (Main.expertMode)
				{
					...
					if (this.aiSECOND % 250 == 0)
					{
						...
						else
						{
							...
							NPC.NewNPC((int)x, (int)y, 508, 0, (float)npc.whoAmI, 0.0f, 0.0f, 0.0f, 255);
							float newX = x + (float)Main.rand.Next(-50, 50);
							float newY = y + (float)Main.rand.Next(-50, 50);
							for (int k = 0; k < 50; k++)
							{
							...
							NPC.NewNPC((int)newX, (int)newY, 509, 0, (float)npc.whoAmI, 0.0f, 0.0f, 0.0f, 255);
						}
					}
					npc.netUpdate = true;
					...
					if (player.position.Y > npc.position.Y + 450.0f)
						npc.velocity.Y = npc.velocity.Y - ((npc.velocity.Y < 0.0f) ? 0.8f : 0.07f);
					if (npc.spriteDirection == -1 && (double)npc.velocity.X > -12.0)
					{
						npc.velocity.X -= 0.1f;
					...
					else if (npc.spriteDirection == 1 && (double)npc.velocity.X < 12.0)
					{
						npc.velocity.X += 0.1f;
						if ((double)npc.velocity.X < -12.0)
							npc.velocity.X += 0.1f;
